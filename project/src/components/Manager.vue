  ...
  <div class="manager">
    <div class="content">
      ...
      <el-table
        ...
        <el-table-column label="管理组" prop="group">
        </el-table-column>
    ...
    <el-dialog title="添加管理员" :visible.sync="dialogFormVisible">
      <el-form :model="form">
        ...
        <el-form-item label="密码" :label-width="formLabelWidth">
          <el-input v-model="form.password" type="password" placeholder="请输入密码（若不填则为不修改）" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="确认密码" :label-width="formLabelWidth">
          <el-input v-model="form.conPassword" type='password' placeholder="请确认密码..." autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
  ...
  export default {
    ...
    methods: {
      ...
      handleEdit(row) {
        ...
        if (username !== row.username && localStorage.getItem("group") != 1) {
          this.$message({
            type: 'info',
            message: '你无权操作该用户'
          });
      ...
      handleDelete(row) {
        ...
        this.$confirm('此操作将删除该管理员, 是否继续?', '提示', {
          ...
          //调删除分类接口
          this.$axios.post('/tank/users/managerDel',param).then((res) =>{
      ...
      timeTrs(val){
        let t = val ? (val.substr(0, 4) + '-' + val.substr(4, 2) + '-' + val.substr(6, 2) + ' ' + val.substr(8, 2) + ':' + val.substr(10, 2)) : null
        return t      
      },
      // 获取管理员信息列表
      getData() {
