...
AbstractTank.prototype = { //公共方法，使用原型的方式
    ...
    stopMoving: function() {
        var _d = this.movingToward,
            thisTank = $('div#' + this.id);
...
EnemyGTank.prototype.isShot = (function() { // 我设计的牛逼坦克，哈哈，打他会还手的
    ...
    var methods = [
        ...
        function() {
            $('#' + this.id).css('background-image', 'url(images/etank/tank4-3.png)');
            counterattack.call(this);
        },
        function() {
            $('#' + this.id).css('background-image', 'url(images/etank/tank4-2.png)');
            counterattack.call(this);
        },
        function() {
            $('#' + this.id).css('background-image', 'url(images/etank/tank4-1.png)');
            counterattack.call(this);
        }
    ]
